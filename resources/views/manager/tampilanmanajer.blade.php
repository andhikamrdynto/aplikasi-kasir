...
<div class="card">
    <div class="card-body">
        <a style="padding: 10px 20px;" href="{{ url('manager/create') }}"
            class="btn icon-left btn-primary mb-4"><span>Tambah</span></a>
                ...
                <tr>
                    ...
                    <td style="display: flex">
                        <div class="dis d-flex">
                            <a href="{{ url('manager/'.$item->id.'/edit') }}" class="btn btn-icon btn-warning ms-1"><i
                                    class="fas fa-pen"></i></a>
                            <form action="{{ url('manager',$item->id) }}" method="POST">
                                @csrf
                                @method('delete')
                                <button type="submit" class="btn btn-icon btn-danger delete ms-1"
                                    data-name="{{ $item->nama_manager }}"><i class="fas fa-trash"></i></button>
                            </form>
                        </div>
                    </td>
                </tr>
